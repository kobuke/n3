...
export default async function MyQuestsPage() {
    const t = await getTranslations('QuestListPage');
    ...
    if (!quests) {
        return <div className="p-8 text-center text-gray-500">{t('fetch_error')}</div>
    }
                            ...
                            <div key={quest.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className="bg-blue-50 text-blue-700 font-bold px-4 py-2 rounded-full text-sm">
                                        {t('progress', { clear: clearLocs, total: totalLocs })}
                                    </div>
                                </div>

                                <div className="mt-6 border-t pt-6">
                                    <h3 className="font-semibold text-gray-900 mb-4">{t('checkpoints')}</h3>
                                    <div className="space-y-4">
                                            ...
                                            return (
                                                <div key={loc.id} className={`flex items-start p-4 rounded-xl border ${isScanned ? 'bg-green-50 border-green-100' : isLocked ? 'bg-gray-50 border-gray-100 opacity-60' : 'bg-white border-gray-200'}`}>
                                                    ...
                                                    <div>
                                                        ...
                                                        {isLocked && !isScanned && (
                                                            <p className="text-xs text-red-500 mt-1 mt-2">{t('sequential_hint')}</p>
                                                        )}
                                                    </div>
                                                </div>
                                            )
                                        ...
                                        {locations.length === 0 && (
                                            <p className="text-sm text-gray-400">{t('no_checkpoints')}</p>
                                        )}
                                    </div>
                                </div>
                            </div>
