...
import { NFT_TYPE_KEYS } from "@/lib/nft-constants";
...
function MyNFTsContent() {
    const t = useTranslations('MyNFTsPage');
    ...
    const CATEGORY_FILTERS = [
        { key: "all", label: t('filter.count', { count: nfts.length }) },
        { key: "certificate", label: tTypes(NFT_TYPE_KEYS.certificate) },
        { key: "more", label: tTypes(NFT_TYPE_KEYS.more) },
        { key: "experience", label: tTypes(NFT_TYPE_KEYS.experience) },
        { key: "asset", label: tTypes(NFT_TYPE_KEYS.asset) },
        { key: "art", label: tTypes(NFT_TYPE_KEYS.art) },
        { key: "other", label: tTypes(NFT_TYPE_KEYS.other) },
    ];
            ...
            <main className="max-w-lg mx-auto px-4 py-6">
                ...
                {!nftLoading && (!filteredNfts || filteredNfts.length === 0) && (
                    <div className="rounded-xl border border-dashed border-slate-200 bg-white p-8 text-center flex flex-col items-center gap-4">
                        ...
                        {categoryFilter === "all" ? (
                            <>
                                <h3 className="font-semibold text-slate-700 text-sm">{t('empty.no_items')}</h3>
                                <p className="text-xs text-slate-400 leading-relaxed max-w-[240px]">
                                    {t('empty.no_items_desc')}
                                </p>
                            </>
                        ) : (
                            <>
                                <h3 className="font-semibold text-slate-700 text-sm">{t('empty.no_category_items')}</h3>
                                <button
