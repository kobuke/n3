...
export default function NotificationsPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-28">
            <AppHeader title={t('title')} showBack onBack={() => router.push("/mypage")} />
                    ...
                    <>
                        {recent.length > 0 && (
                            <section className="mb-8">
                                <h3 className="text-slate-700 text-sm font-bold mb-3">{t('recent')}</h3>
                                <div className="space-y-3">
                                    {recent.map(n => (
                                        <NotificationItem 
                                            key={n.id} 
                                            ...
                                            viewDetailText={tCommon('view_detail')}
                                        />
                                    ))}
                                </div>
                            </section>
                        ...
                        {earlier.length > 0 && (
                            <section>
                                <h3 className="text-slate-700 text-sm font-bold mb-3">{t('earlier')}</h3>
                                <div className="space-y-3">
                                    {earlier.map(n => (
                                        <NotificationItem 
                                            key={n.id} 
                        ...
                        {recent.length === 0 && earlier.length > 0 && notifications.length > 0 && (
                            <div className="pt-4 text-center">
                                <p className="text-xs text-slate-300 font-medium">{tCommon('end_of_list')}</p>
                            </div>
                        )}
                    </>
                )}
...
function NotificationItem({ 
            ...
            <div className="flex-1 min-w-0">
                ...
                {n.link_url && (
                    <span className="inline-flex items-center gap-1 mt-2 text-xs text-primary font-medium">
                        {viewDetailText} <ExternalLink className="w-3 h-3" />
                    </span>
                )}
            </div>
            {isNew && (
    ...
    if (n.link_url) {
        return (
            <Link href={n.link_url as any} className="block hover:opacity-90 transition-opacity">
                {content}
            </Link>
        )
    }
